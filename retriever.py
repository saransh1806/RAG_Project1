...
"""
    ...
    return summary
def generate_query_embedding_for_semantic_search(hyde_model,tokenizer,embedding_model,current_query,previous_chat):
...
"""
    ...
    **hyde_inputs,
    ...
    hyde_input_length = hyde_inputs["input_ids"].shape[1]

    hyde_generated_tokens = hyde_outputs[0][hyde_input_length:]

    hyde_document = tokenizer.decode(
    hyde_generated_tokens,
    skip_special_tokens=True
)
    query_embedding=embedding_model.encode(hyde_document,normalize_embeddings=True)
    return query_embedding

def semantic_search(hyde_model,tokenizer,embedding_model,current_query, previous_chat, collection, top_k=50):
